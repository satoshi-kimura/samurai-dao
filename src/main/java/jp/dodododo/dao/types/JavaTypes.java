...
public class JavaTypes<T> implements JavaType<T> {
	...
	public static final class EnumType<T extends Enum<T>> implements JavaType<T> {
		...
		public T getValue(ResultSet rs, int columnIndex) throws SQLException {
			Object value = null;
			if (jp.dodododo.dao.util.EnumConverter.NumKey.class.isAssignableFrom(enumClass)
					&& jp.dodododo.dao.util.EnumConverter.StringKey.class.isAssignableFrom(enumClass)) {
				value = rs.getObject(columnIndex);
			} else if (jp.dodododo.dao.util.EnumConverter.NumKey.class.isAssignableFrom(enumClass)) {
				value = rs.getDouble(columnIndex);
			} else if (jp.dodododo.dao.util.EnumConverter.StringKey.class.isAssignableFrom(enumClass)
					|| jp.dodododo.dao.util.EnumConverter.NameKey.class.isAssignableFrom(enumClass)) {
				value = rs.getString(columnIndex);
			} else if (jp.dodododo.dao.util.EnumConverter.OrdinalKey.class.isAssignableFrom(enumClass)) {
